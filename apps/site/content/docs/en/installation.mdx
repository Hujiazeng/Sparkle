---
title: Installation
description: How to install and set up Sparkle on your system.
---
...
Download the latest release for your platform from [GitHub Releases](https://github.com/Hujiazeng/Sparkle/releases).
...
2. Open the DMG and drag Sparkle to your Applications folder
3. Launch Sparkle from Applications
...
3. Sparkle will be available in your Start menu

## First Launch & Setup Center

On first launch, Sparkle automatically opens the **Setup Center** — a guided setup overlay that walks you through three prerequisites:
...
- **Multiple installations detected** — If Sparkle finds more than one Claude Code binary (e.g., one installed via npm and another natively), it shows a warning with cleanup instructions. See [Claude Code Conflicts](#claude-code-conflicts) below.
...
> **Note:** Skipping the provider step doesn't mean you have a working provider. If you try to send a message without any configured provider, Sparkle will prompt you to set one up.
...
Sparkle needs at least one API provider to function. Go to **Settings** and open the **Providers** section to add one:
...
Sparkle includes an auto-updater. When a new version is available, you'll see a notification in the app. You can also manually check for updates in the **General** section of **Settings**.
