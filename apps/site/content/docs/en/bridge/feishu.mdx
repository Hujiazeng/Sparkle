...
The process is the same as the domestic edition, but performed on [Lark Developer](https://open.larksuite.com). When configuring in Sparkle, select the domain as **Lark**.

## Configure in Sparkle
...
3. Check Sparkle logs for `[feishu/gateway] handleEventData type: card` output
...
3. Check Sparkle logs for `[card-controller]` related output
