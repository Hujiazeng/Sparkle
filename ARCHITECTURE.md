...
Sparkle 是多模型 AI Agent 桌面客户端。Electron 40 做外壳，Next.js 16 (App Router) 做前端和 API 层，better-sqlite3 做本地持久化，通过 Claude Agent SDK 与 AI 服务商交互。
...
Telegram/Feishu 消息
  ...
  → channelRouter 路由到 Sparkle session
...
| `channel_bindings` | Bridge: IM 频道 → Sparkle 会话绑定 |
...
`src/lib/bridge/` — 将外部 IM（Telegram、飞书）连接到 Sparkle 会话。
