...
语音能力作为 **MCP server** 实现，不依赖 Claude Code 的 Channel 协议。参考 Channel 的架构思路（MCP server 桥接外部 IO），但完全由 Sparkle 自己控制。
