...
| 终端 Claude Code 配了 Bedrock，打开 Sparkle 选 OpenRouter | 请求莫名跑到 Bedrock，用户完全无法理解 | ✅ 2026-04-15 通过 per-request shadow `~/.claude/` 实现真正隔离：DB provider 请求时自动剥离 settings.json 的 ANTHROPIC_*，其余 user-level MCP/plugins/hooks/CLAUDE.md 不受影响 |
