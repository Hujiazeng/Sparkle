...
| NEXT-PA `defaultExpanded is not defined` | 1x | 1x（仅出现在 0.50.2）| 12:55 | `Sparkle_08eujv0._.js:405 FileTree` |
...
- 推断：auto 模式在 cc-switch 用户 / 全新用户场景下**错选 Native**，Native 没有读 `~/.claude/settings.json` 的能力，遇到"DB 无 provider + env 无 ANTHROPIC_* + 用户未登录 Sparkle 但已装 cc-switch"时抛错
...
- 未直接验证：没有反编译 0.50.2 的 `Sparkle_08eujv0._.js:405` bundle，没有定位到 Turbopack 具体版本的已知 bug
...
- 用户 `~/.claude/settings.json` 有 env 块（cc-switch 写入），Sparkle DB 里 0 个 provider
...
2. **没有**在 Sparkle 里添加任何 DB provider
...
- 方案 B：**会**触发（注意：之前文档旧版写的"不触发"是**错的**，与 Phase B 自己的 `hasSparkleProvider()` 契约矛盾——Codex 核查结论 2026-04-15 指出并修正）。Phase B 契约明确规定 `hasSparkleProvider()` **不**读 `~/.claude/settings.json`，只看 DB provider / process.env ANTHROPIC_\* / legacy `anthropic_auth_token` setting。纯 cc-switch 用户这三者都为空 → 会被入口拦截 → 弹 SetupCenter Provider 卡片。
  - 对 cc-switch **正在挂掉**的用户（#493 问题二场景）：拦截引导他们"去 Sparkle 里加个 provider"——从**结果上恰好**绕开了 cc-switch 故障（只要他们按引导加了一个 DB provider，后续走 DB provider 路径就不再碰 cc-switch 代理了）。这是副作用不是有意识别，诊断意义为零但体验结果是正向的
  - 对 cc-switch **正常工作**的用户：也会被拦截一次（这是 by design——符合用户"CLI 登录状态不管，只看 Sparkle provider"的明确约定），需要他们在 Sparkle 里显式加一个 provider。**这是 Phase B 的主要行为变化之一**，发版说明里要**明确告知**现有 cc-switch 用户
- 方案 C：即便 Phase B 没拦住（例如用户已加 DB provider 但那个 provider 走 cc-switch 代理），SDK spawn 后仍会 `PROCESS_CRASH`。方案 C 能把此 crash 归一为"去设置 → 服务商"——但**这个引导对 cc-switch 代理挂掉的用户仍是错的答案**：他在 Sparkle 里有 provider，他缺的是"cc-switch 代理没跑"或"token 失效"这类信息
...
2. `error-classifier.ts` 新增分类 `CC_SWITCH_PROXY_UNREACHABLE`，pattern 识别 CLI `code 1` + 上述 tag，文案改成"检测到 cc-switch 代理模式，请确认 cc-switch App 正在运行且代理端口与 settings.json 匹配"——**不**引导去 Sparkle 设置
...
- **只看"Sparkle 是否有可用服务商"**，不看 CLI 登录状态
...
1. 新增 `src/lib/provider-presence.ts`（或加在 `claude-settings.ts`）：
   ```ts
   export function hasSparkleProvider(): boolean {
     // DB provider 任一记录 ≥ 1
     // OR env vars
     // OR legacy setting
   }
   ```
   注意：**不包含** `hasClaudeSettingsCredentials()`——cc-switch 的 settings.json 不视作"Sparkle 的服务商"。

2. `src/app/api/chat/route.ts:24-48` 在 `getSession` 之后、`acquireSessionLock` 之前插入：
   ```ts
   if (!hasSparkleProvider()) {
...
**回归风险**：
- **现有 cc-switch 用户升级后首次发消息一定会被拦截**（by design，但需要发版说明明确告知）。用户画像：只靠 `~/.claude/settings.json` 管 auth、从未在 Sparkle 里添加 provider。按 Phase B 契约 `hasSparkleProvider()` 不读 settings.json，这部分用户会被拦去 SetupCenter Provider 卡片。缓解：0.50.3 发版说明加一段"如果你用 cc-switch 管 Claude Code 凭据，升级后 Sparkle 会要求你在 设置→服务商 里显式添加一个 provider——这是为了让 Sparkle 能正确处理第三方 provider 的隔离。你仍可以通过 cc-switch 管理 Claude Code CLI 本身的凭据"。
- 极少数用户只装了 CLI、在 CLI 里登录了、**从未在 Sparkle 添加任何 provider**：按新判定会被拦截。
  - 缓解：`/api/setup/route.ts` 当前判定把"装了 CLI"也算 provider `completed`——新拦截逻辑与此**有意不一致**（入口拦截要求 Sparkle 里有真正 provider）。两条口径要各自注释为什么。
...
- Claude Code CLI 抛 `Not logged in · Please run /login`（CLI 侧文案，Sparkle 原样透传）
...
2. `src/lib/error-classifier.ts:178-179` `userMessage` / `actionHint` 在 i18n 层面改成明确"Sparkle 语境"：
   - `userMessage`: `No provider configured in Sparkle.`
...
- i18n 文案如果改成 "Sparkle-specific" 后，一些用英文 UI 的开发用户可能觉得文案不够技术化——需要平衡
...
**需核实 / 可能的微调**：
- 当前 `/api/setup/route.ts` 把"CLI 存在"也算 provider `completed`（第 40-46 行），这条口径与方案 B 入口拦截的判定**不一致**（B 不把 CLI 存在算作 "Sparkle provider"）
- **建议**：保持 `/api/setup` 现状不变（它控制 SetupCenter 的三个卡片状态，CLI 算 `completed` 是合理的），但新增一个 `/api/setup/provider-ready` 或在 `hasSparkleProvider()` 函数里用更严格口径。两条判定口径要各自注释清楚为什么不同。

---

## 方案之外：百炼渠道 catalog 更新（独立 0.50.3 hotfix，关联 #483）

**Issue**：[#483](https://github.com/op7418/Sparkle/issues/483) 阿里云 codingplan 没有 `qwen3.6-plus` 模型。仓库作者 2026-04-15 12:54 UTC 评论"这个还没加，我加一下"。
...
- **2026-04-15**：方案确定
  ...
  - 用户明确"CLI 登录状态不管"——入口拦截仅看 Sparkle provider 存在性
...
| #493 问题二 · Phase B 是否触发 | **FAIL**（文档自相矛盾） | Phase B 契约明确 `hasSparkleProvider()` 不读 settings.json，纯 cc-switch 用户**会**被拦截；但 #493 小节旧版写的是"不触发" | 已修正 §"方案之外：#493 真实情况 · 问题二 · Phase B"段落，同步更新 Phase B 回归风险（明确告知现有 cc-switch 用户升级后会被拦一次）|
