...
Sparkle 发展至今，积累了 6 套独立的"连接器"系统，各有自己的发现机制、注入方式、调用链路和 UI 入口：
...
映射到 Sparkle：应有一个统一的能力层（Capability Layer），所有扩展注册进去，Claude 按需调用。
...
**方案**：Delta 模式。
- `loadSparkleMcpServers()`：只返回有 `${...}` env placeholder 的服务器（需从 Sparkle DB 解析）。当前实际配置中无此类服务器，返回 undefined。
...
参考 Perplexity 桌面版的"常驻后台 + 快捷键拉起"模式，Sparkle 的浮窗助理定位为：
...
| 项目 | 说明 | 优先级 |
|------|------|--------|
| Skills/Commands 双轨发现 | SDK 自动发现 + Sparkle 手动扫描不同步，UI 显示的能力 ≠ Claude 实际知道的 | P1 |
