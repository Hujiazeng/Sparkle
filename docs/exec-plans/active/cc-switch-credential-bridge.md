...
> Issues：[#461](https://github.com/op7418/Sparkle/issues/461)、[#478](https://github.com/op7418/Sparkle/issues/478)、[#476](https://github.com/op7418/Sparkle/issues/476)、[#457](https://github.com/op7418/Sparkle/issues/457)、[#470](https://github.com/op7418/Sparkle/issues/470)
...
- Sparkle 侧没有在 UI 里配 provider（或配了也不是 cc-switch 那一套）
- 终端直接跑 `claude` 可用
- 在 Sparkle 里发消息报 "No provider credentials available"
...
**cc-switch 不设 shell env、不设 system env、不写 Sparkle DB。** 一切都走 Claude Code CLI 的原生配置通道。
...
function hasCredentialsForRequest(providerId?: string): boolean {
  ...
  if (getSetting('anthropic_auth_token')) return true;  // Sparkle 自己的 DB
...
2. `getSetting()` 读 Sparkle 自己的 DB（`sdk_settings` 表）
3. Sparkle DB 的 `providers` 表
...
```ts
// Prevent ~/.claude/settings.json from overriding Sparkle's provider configuration.
...
Switch to Claude Code SDK runtime in Settings, or add the provider to Sparkle directly."
```
...
**前置：** 清空 Sparkle 的所有 provider + 清空 shell env 的 `ANTHROPIC_*`。
...
4. 打开 Sparkle，启动新对话，发 "who are you"
...
- **不做** model 名从 settings.json 恢复（SDK 黑名单挡住，且 Sparkle 已有 model 选择器，不是痛点）
...
- **2026-04-15 review 后跟进** — code review 指出删了 MANAGED_BY_HOST 之后没有任何机制阻止 SDK 把 `~/.claude/settings.json` 的 env 块应用到子进程：SDK 的 `qZq()` 在 `settingSources` 含 `'user'` 时会把 settings.env 写进 process.env，覆盖掉 Sparkle spawn 时注入的 provider auth。初次补丁：**DB-backed provider 的 `settingSources` 改为 `['project', 'local']`（不含 'user'）**
...
- **最终方案（实施）：per-request shadow `~/.claude/`**
  ...
  - SDK 子进程的 HOME / USERPROFILE 指向 shadow root → SDK 的 `qZq()` 读到剥离版 settings.json → 不再覆盖 Sparkle 注入的 provider auth
...
- **2026-04-15 第六轮 review：drop 'project' 把项目 `.mcp.json` 也搂走了**
  ...
  - 名字冲突时优先级：Sparkle UI 管理 / placeholder 解析后的 servers > 项目 `.mcp.json`（用户当前选择层 > 团队默认层）
...
- **2026-04-15 第七轮 review：`loadProjectMcpServers` 漏了 `mcpServerOverrides`**
  - reviewer 指出我新写的 helper 没有应用 `~/.claude/settings.json` 的 `mcpServerOverrides`——这是 Sparkle MCP Manager UI 持久化"我把这个 server 关了"或"我把这个 server 强制开"的位置（原始 `loadAndMerge` 是有这层处理的，见 `mcp-loader.ts:57-62`）
