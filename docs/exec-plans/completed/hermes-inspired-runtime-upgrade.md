...
- **Sparkle 项目根约定**：`/Users/op7418/Documents/code/opus-4.6-test-hermes-impl/CLAUDE.md`
...
- Sparkle agent-loop：`src/lib/agent-loop.ts:225`、`:353-387`（for await fullStream 消费 tool events）
...
- Sparkle 既有 roleModels：`src/lib/provider-catalog.ts:41, :73-75`、`provider-resolver.ts:262-278`
...
- §2.3 改为 "Sparkle 有两层压缩：微观 per-step 的 `pruneOldToolResults`（context-pruner.ts）+ 宏观 per-chat-turn 的 `compressConversation`（context-compressor.ts，wire 在 chat/route.ts）"
