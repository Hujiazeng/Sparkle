...
3. **新建和编辑 Markdown 文档** — 不用切到 Obsidian 即可在 Sparkle 内新建 `.md` 并以源码模式编辑，兼容已有 vault。
...
**2026-04-21 复查结论：跳过。** craft-agents 的 `ShikiThemeContext` 解决的是"深色专属主题在浅色系统模式下 fallback 错乱"——Sparkle 的 `code-block.tsx:463-467` 已经走 `useThemeFamily` + `resolveShikiThemes` 显式传 [light, dark] 给 Shiki 的 `codeToTokens`，生成 dual-theme 输出后由 CSS 变量（`:root.dark`）切换，**不依赖 DOM class 嗅探**。额外加 Provider 是重复造轮子。若未来有新主题族引入，只需扩 `theme/code-themes.ts` 即可，无需新 Context。
