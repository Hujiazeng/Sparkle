...
**目标：** 应用自身的 UI 文案使用 "Sparkle" 而非 "Claude"（Claude Code 连接部分除外）。

### 已实现

- `src/i18n/en.ts`：`Claude Chat` → `Sparkle Chat`，`conversation with Claude` → `conversation with Sparkle`
- `src/i18n/zh.ts`：`Claude 对话` → `Sparkle 对话`，`与 Claude 对话` → `与 Sparkle 对话`
