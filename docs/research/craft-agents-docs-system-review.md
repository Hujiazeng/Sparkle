...
> 目的：考察一个同品类开源项目（Agent 桌面客户端）如何组织文档，产出 Sparkle 文档体系可借鉴的具体动作清单。
> 三层结构：**[craft 事实]** 带 `file:line`；**[Sparkle 事实]** 带仓库内路径；**[推断]** 标注。
>
...
Sparkle 已经形成相对完整的内部文档链（`exec-plans/` + `research/` + `handover/` + `insights/`），但面向**外部用户与社区贡献者**的文档尚薄弱（无 Issue 模板、无 PR 模板、README 未嵌入项目结构图）。本次对标选 craft-agents-oss，因为它与 Sparkle 定位几乎重合（Electron Agent 客户端 + CLI + 远程服务端），但走的是公开开源路线，面向外部读者的治理文档更完整。

对标目标：
1. 抽出 craft 做得好而 Sparkle 缺失的"外部入口"文档形态。
2. 确认 Sparkle 现有"内部研发"文档链是否真的更强（避免反向学习）。
3. 产出可执行的借鉴清单。
...
Sparkle 在"研发过程沉淀"上比 craft 明显更重：
...
### 4.1 craft 强于 Sparkle（应借鉴）

| 维度 | craft 做法 | Sparkle 现状 |
|------|-----------|---------------|
...
### 4.2 Sparkle 强于 craft（应保持）

| 维度 | Sparkle 做法 | craft 现状 |
|------|---------------|-----------|
...
**结论**：Sparkle 的"内功"文档链（研发过程沉淀）已显著强于 craft，**不需要向 craft 反向学习**。

### 4.3 两边都缺的项

- ADR（架构决策记录）独立目录。Sparkle 把决策散落在 `exec-plans/` 里，craft 完全不记。
- CHANGELOG 机器可读化。Sparkle 有 `RELEASE_NOTES.md` 模板但仍是人工维护，craft 无。
...
目标：Sparkle 已公开发布（观察到 `RELEASE_NOTES.md` 正在用 GitHub Release 分发 DMG/Setup），但 Issue 质量与社区协作缺乏结构化约束。

1. **新增 `.github/ISSUE_TEMPLATE/bug_report.yml`**：模仿 `craft/.github/ISSUE_TEMPLATE/bug_report.yml:1-124` 的字段，收集 Sparkle 版本、OS（含 arm64/x64）、Provider、复现步骤、日志位置。
2. **新增 `.github/ISSUE_TEMPLATE/feature_request.yml`**：结构 Problem / Proposed Solution / Alternatives / Additional Context。
3. **新增 `.github/pull_request_template.md`**：列自查清单，对齐 `CLAUDE.md` 的"改动自查"五条（i18n、DB schema、类型、文档、功能文档）。这是 craft **没做** 但 Sparkle **应做** 的，因为我们有更细的自查纪律。
...
7. **在 `src/` 主模块目录增加简短 README**（按需，不求全）：仅对外部读者最关心的模块（如 `agent-sdk`、`claude-client`、`chat`）写 50–100 行"本模块做什么 + 常见坑点 3 条"的聚焦文档，参考 `apps/electron/README.md:49-125` 浓缩风格。Sparkle 非 monorepo，不必覆盖所有目录。
...
11. **文档链接校验 CI**：用 `markdown-link-check` 或自建脚本，扫 `docs/` 下所有 `file:line` 引用是否仍存在。两边仓库都缺，Sparkle 率先补上。

### 5.4 不采纳的 craft 做法

- **`TRADEMARK.md`**：Sparkle 未明确开源路线，暂不需要。未来若对外开源再补。
- **把所有内部库都免 README**：craft 对 `packages/shared` / `packages/server` 直接不写文档，不符合 Sparkle 的"每个核心模块要有可交接的文档"纪律。不借鉴。
- **ASCII-only 图示**：craft 完全不用 Mermaid。Sparkle 的 `handover/` 已有 Mermaid 流程图惯例（[推断] 需核验），保持现状更清晰。

---

## 6. 未决事项

1. Sparkle 根目录 `README.md` 当前结构是否已嵌入架构树 / Quick Start / Troubleshooting —— 本次调研未打开 `README.md`，P1 建议 6 落地前需先核验。
