...
**关键发现：Sparkle 已经有一个完整的 DB-based fallback 路径（`buildFallbackContext`），SDK resume 只是一个优化而非必需。**
...
**Sparkle 参考价值**：Sparkle 已有 `microCompactMessage()` 函数（在 `buildFallbackContext` 中使用），但仅做简单截断。可以增强为基于工具名的选择性清理。
...
**位置**：Sparkle 的 `claude-client.ts` 已有实现
...
Sparkle 已有此实现（`context-compressor.ts` + `claude-client.ts` 的 CONTEXT_TOO_LONG 处理）。脱离 SDK 后保持不变，但需要：
- 改为直接调用 Anthropic API（而非 `generateTextViaSdk`）
- 压缩后直接重建 messages 数组重试

### 4.2 与 Sparkle 现有机制的关系
...
| 维度 | Claude Code | OpenCode | Codex | Sparkle（当前） |
|------|------------|---------|-------|-----------------|
