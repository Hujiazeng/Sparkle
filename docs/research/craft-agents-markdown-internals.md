...
> 姊妹文档：
> - [markdown-editor-tiptap-evaluation.md](./markdown-editor-tiptap-evaluation.md) — Sparkle 自己的 Markdown 栈与 Tiptap 可行性
...
>
> 三层结构：**[craft 事实]** 带 `file:line`；**[Sparkle 事实]** 带本仓库路径；**[推断]** 标注。

---

## 1. 调研动机

craft 是"文档驱动的 Agent"——Skills / Agent 配置 / Source Guides / Session 输出全部以 Markdown 文件承载；聊天窗口本身也是 Markdown 渲染器。和 Sparkle 现状高度可对照。前一份 Markdown 调研（`markdown-editor-tiptap-evaluation.md`）中留下的三个关键问题，正好能用 craft 的实战选型来交叉验证：
...
- 对比：Sparkle 现状 `src/components/ai-elements/message.tsx:349` 的 memo 只比 `children` 引用，没有 id 维度。长消息 + 多消息列表时缓存粒度偏粗。
...
- [推断] 由于启用了 `rehype-raw`（允许原始 HTML），需要这层防御否则 LLM 一旦输出怪标签会炸。Sparkle 的 `streamdown` 对类似问题默认保守转义，严格性不同但都需要考虑。
...
- [推断] 很实用的长文档阅读体验，相当于 Obsidian / Notion 的折叠。Sparkle 的 Streamdown 默认无此能力，可作为独立扩展点。
...
- [推断] 和 Sparkle 的 Skills 设计几乎一致，互相印证此模式是事实上的行业惯例。
...
- [推断] 选型决策因子是**目标用户**：craft 面向偏产品向用户（类 Notion 体验），Sparkle 面向创作者 + 开发者（需要源码模式），两者不同。前文推荐 CodeMirror 6 对 Sparkle 仍然成立，但应补一句"若未来做面向非技术用户的笔记场景，Tiptap 是备选"。
...
- craft 的 memo 要求调用方传 `id`，形成"消息 ID 维度"的缓存边界。Sparkle 当前 `src/components/ai-elements/message.tsx:338-350` 的 memo 只比 `children` 引用。
...
2. **核对 Streamdown 渲染链是否复用 `code-block.tsx:163-170` 的 LRU 缓存**（先验证再决定动作）
   - Sparkle 已有 `highlighterCache` + `tokensCache`，但不确定消息流里的代码块是否走同一条高亮通路。若未复用，则对齐；若已复用，本条无需改动。
...
5. **`MarkdownDiffBlock` 接入**（0.5 天）
   - 证据：`MarkdownDiffBlock.tsx:1-60` + `@pierre/diffs`；Sparkle git diff 展示可直接复用。
...
9. **Skills frontmatter 字段规范对齐**（核对即可）
   - 证据：`packages/shared/src/skills/storage.ts:68-95`；Sparkle 已有 frontmatter 解析，比对字段集查漏。
...
1. 代码块 language 拦截的完整语言清单需要与 Sparkle 产品侧对齐（哪些必要，哪些暂缓）。
2. LLM 提示词需要同步学会输出魔法 language；需落一份 system prompt 说明。
3. craft 的 `markitdown-js` 做"外部文档 → Markdown"转换，Sparkle 是否需要对应能力（PDF 拖进来自动转 Markdown）需产品判断。
4. 长会话虚拟化 craft 也没做，此问题仍需 Sparkle 独立解决，参考前份 `markdown-editor-tiptap-evaluation.md` 第 5.2 节。
