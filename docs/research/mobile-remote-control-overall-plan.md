...
> 创建时间：2026-03-12
> 最后更新：2026-03-13
> 适用范围：Sparkle 桌面端 + Android Companion + 多设备控制 + Feishu 能力扩展

## 一、这次更新后的结论

在引入 OpenClaw 官方飞书插件和 `claude-to-im-skill` 作为参考后，Sparkle 的远程方案应从“桌面 Bridge + 移动端控制器”升级为三层结构：
...
- 当前 Sparkle 已有桌面 Bridge、远程审批、流式输出、文件预览和多 IM 通道
...
这说明飞书这条线在 Sparkle 中也不应继续停留在“一个 adapter 文件 + 几个设置项”的层级。
...
这说明 Sparkle 后续在做远程能力升级时，应该把“桌面主产品”和“轻量守护进程/skill 包装层”视为两个不同交付面。
...
- `Host`
  一台运行 Sparkle 的执行主机。负责工作目录、数据库、Claude CLI、Provider 凭证和会话运行。
- `Controller`
...
### 风险三：`claude-to-im-skill` 的轻量假设和 Sparkle 主产品冲突
