...
- Tiptap 是否适合做 Sparkle 的 Markdown 渲染/编辑？
- 如果不是 Tiptap，替代路径是什么？
...
- **商业影响**：若要在 Sparkle 做 Tiptap + 流式 AI 写入，需订阅 Start plan。此前表述"streamContent 是开源核心"低估了商业风险，在此更正。
...
2. **与"忠实展示 CLI / Obsidian 内容"的定位冲突**：Markdown 往返已知有损（frontmatter、callouts、嵌套、HTML 注释），Sparkle 作为面向开发者和创作者的客户端，不应在文件层引入 lossy 转换。
