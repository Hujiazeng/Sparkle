# OpenClaw 微信插件拆包与 Sparkle 逆向集成可行性调研
...
可以集成，但不建议把 `@tencent-weixin/openclaw-weixin` 这个 npm 包直接塞进 Sparkle 运行。

更合适的做法是：把它当成协议说明和参考实现，按 Sparkle 现有 `Bridge` 架构原生实现一个 `weixin` adapter。
...
- **直接安装到 Sparkle 里当依赖运行：不现实**
- **按协议和实现样本重写一个 Sparkle 原生 adapter：可行**
...
而 Sparkle 当前 bridge 设置还是以平面 key-value 为主，例如：
...
所以“npm install 然后 require 进 Sparkle”这条路不值得走。

## 七、与 Sparkle 现状的匹配度

### 1. 能直接复用的现有能力

Sparkle 已有这些基础能力，能接住微信适配器：
...
- 运行时耦合太深
- 会把 OpenClaw runtime 假设硬塞进 Sparkle
- 后续调试成本会很高
...
- **把腾讯这套包当协议参考，不当运行时依赖**
- **用 Sparkle 自己的 bridge 架构重写微信 adapter**
