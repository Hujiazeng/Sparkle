...
## 二、Sparkle 当前的 Skills 实现

### 2.1 现有架构

Sparkle 有完整的 Skills 管理系统，但**大量依赖 SDK**：
...
| 维度 | Claude Code | OpenCode | Craft Agent | Sparkle 现状 |
|------|------------|----------|-------------|---------------|
...
保持与 Claude Code 兼容的 `SKILL.md` 格式，增加 Sparkle 特有扩展：
