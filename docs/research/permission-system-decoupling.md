...
## 1. Sparkle 当前权限流程的完整链路

### 1.1 当前架构：完全依赖 SDK

Sparkle 当前的权限系统 **100% 依赖 Claude Code SDK**。自身不做任何权限判定，仅负责"转发审批请求 + 收集用户决策 + 回传结果"。
...
**复杂度评估：** 极高。涉及 AI 分类器、deny tracking、hook 系统、多来源规则合并、MCP 工具名前缀处理等。Sparkle 不需要复刻全部。
...
| **Sparkle 适用性** | 过于复杂 | 规则引擎可借鉴 | 整体架构最适合 |
...
4. **不搞 AI 分类器**：Claude Code 的 auto 模式分类器成本高、延迟大、对 Sparkle 场景价值有限
