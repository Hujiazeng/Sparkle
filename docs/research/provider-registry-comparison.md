# Provider 注册表对比分析 — Hermes vs Sparkle

> 分析日期：2026-04-09

## 为什么这个问题重要

Sparkle 的 provider 系统频繁出问题：新增 provider 时忘记更新多处代码、auth 方式判断散落在多个文件、模型能力（thinking/vision/tool use）靠硬编码猜测、第三方代理兼容性需要逐个调试。根本原因是**数据源和判断逻辑没有统一**。

## 两套系统的核心区别

### Sparkle 现状：一切硬编码在 VENDOR_PRESETS
...
**Sparkle 优势：** Zod schema 验证 preset 完整性；UI 引导式配置；DB 持久化。
...
**Sparkle 优势：** bedrock/vertex 确实有特殊的 SDK（@ai-sdk/amazon-bedrock 等），需要独立处理，不能简单归入 openai_chat。
...
**Sparkle 问题：** auth 判断散落在 `inferAuthStyleFromLegacy()`、`resolveAnthropicAuth()`、`toClaudeCodeEnv()` 多处，新增 auth 方式需要改多个函数。
...
**Sparkle 问题：** `getContextWindow()` 是一个巨大的 switch/case，每次新模型都要手动加。`supportsEffort` 只在 models route 里某些模型手动标记。没有 vision/tool_call/structured_output 的统一标记。
