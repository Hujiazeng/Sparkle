...
> 调研目标：了解 Claude Code、OpenCode、Codex、Craft Agents 的子 Agent 实现，为 Sparkle 脱离 SDK 后自建子 Agent 系统提供方案。
...
Sparkle 当前使用 Vercel AI SDK 的 `streamText` 进行 LLM 调用。子 Agent 本质上是：
