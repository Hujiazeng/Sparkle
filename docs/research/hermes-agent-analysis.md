# Hermes Agent 分析 — 与 Sparkle Native Runtime 的结合机会

> **本文采用三段式结构**：外部事实（Hermes）/ 本仓库事实（Sparkle, file:line）/ 推断与建议。
> 每条陈述必须可追溯到具体来源；推断层不得与事实层混写。
>
> **⚠️ 对比基线说明**：本文的 Sparkle 现状以 [`docs/handover/decouple-native-runtime.md`](../handover/decouple-native-runtime.md) 为权威来源。
...
**关键事实**：
- 每个 provider 预设都可以配 `roleModels.small` 或 `roleModels.haiku`——这些已经是 Sparkle 既有的架构约定，用户在 provider 编辑界面能直接改
...
- **退化映射也被考虑过**：MiniMax / MiMo 这类无独立小模型的 provider 把 `haiku/sonnet/opus` 全部映射到同一个模型（`provider-catalog.ts:370-405`）——Sparkle 早就接受"不是所有 provider 都有便宜档"这个事实
...
**依据**：Hermes §1.3（四层 whitelist-first 判定） + Sparkle §2.1（AI SDK 已并行但无安全判定）
...
- Sparkle §2.6（sdkProxyOnly 硬约束）
- Sparkle §2.7（`roleModels.small` / `.haiku` 槽位早已存在）
...
**依据**：Hermes §1.5（`SubdirectoryHintTracker` 参考实现）+ Sparkle §2.5（只有 cwd + parent 两级发现）
...
- Sparkle §2.4：memory_search 已成熟但只搜工作区文件；messages 表完全没有搜索工具
...
**排在 LLM 压缩之前的三个理由**：
- Sparkle §2.3 的死代码信号：长上下文退化还不是活跃痛点
- 复用现有工具壳，成本比从零做 LLM 压缩低一个量级
- 对"找回以前对话里讨论过什么"这种诉求，session 搜索是**直接答案**，不是迂回优化

### 3.5 P2 — LLM 驱动上下文压缩（分两步走）

**依据**：Hermes §1.6（ContextCompressor 5 步算法）+ Sparkle §2.2-2.3（固定截断 + 死代码）
...
**依据**：Hermes 在复杂任务完成后主动 nudge 用户保存 Skill；Sparkle 有 Skill 系统但只做发现/执行
...
Sparkle 是双 runtime 架构（见 `docs/handover/decouple-native-runtime.md`）：
...
| 多平台 IM 网关（Telegram/Discord/Slack/WhatsApp/Signal/Matrix/钉钉/飞书 8 家） | Sparkle 有独立的 bridge 系统，架构不同；8 家支持不是 runtime 质量瓶颈 |
...
2. **复用 Sparkle 既有的 `roleModels.small` / `.haiku` 槽位**：原 §3.2 step 2 写"主 provider 的小模型（Anthropic → Haiku、OpenAI → gpt-mini）"，硬编码成 provider-specific 启发式。修正为直接读 `resolved.roleModels.small` / `.haiku`——这两个槽位早就存在于 `provider-catalog.ts:41, :73-75`，且 `provider-resolver.ts:268-272` 已经在为 SDK 路径把 `.small` 写到 `ANTHROPIC_SMALL_FAST_MODEL` 环境变量。Native Runtime 下只是没读。新增 §2.7 记录这个事实。
