...
调研 Vercel 最新推出的 Chat SDK（`npm install chat`）是否能集成到 Sparkle 中，实现通过 Telegram 等 IM 远程控制 AI 应用。
...
**关键点**：Sparkle 的核心是 Claude Agent SDK 驱动的**本地代码执行引擎**，不是简单的 LLM 聊天。它涉及文件读写、终端命令、权限审批等本地操作。
...
| **富内容展示** | Sparkle 的 UI 包含代码高亮、diff 预览、文件树、终端输出、思维链等，Telegram 仅支持纯文本 + Markdown |
...
| **部署模型不同** | Chat SDK 需要一个常驻服务端来接收 webhook，而 Sparkle 是桌面应用 |
...
Chat SDK 本质是让 **服务端 bot** 部署到多个平台。而 Sparkle 是**本地桌面应用**直接操作用户机器上的文件。这两个模型之间存在根本性的架构鸿沟。

## 四、结论

**不建议集成 Chat SDK 到 Sparkle。** 原因：

1. **用途不匹配**：Chat SDK 解决的是「一个 bot 部署到多个聊天平台」的问题，而 Sparkle 是本地代码执行工具，不是聊天 bot
2. **改造成本极高**：需要引入云端中转服务、远程文件访问、简化版权限审批、消息格式降级等大量基础设施
3. **体验必然降级**：Telegram 无法承载 Sparkle 当前的富交互体验（代码 diff、文件树、工具审批等）
4. **安全风险**：通过 Telegram 远程触发本地文件操作和终端命令，需要非常谨慎的安全设计

### 如果确实想要远程控制能力

更合理的路径是：
- **方案 A**：为 Sparkle 加一个 Web 远程访问层（类似 VS Code Remote），保留完整 UI 体验
