...
这个方案在 Claude.ai 的架构下工作得很好，但在 Sparkle 的场景下有几个问题：

1. **SDK 限制**：Sparkle 使用 Claude Agent SDK 的 `preset: 'claude_code'` 模式，无法注册自定义 tool。SDK 暴露的是 text delta 流，不支持在 tool 层面扩展。
...
这个格式复用了 Sparkle 已有的代码围栏模式（`image-gen-request`、`batch-plan` 等），前端的 parser 链天然支持。
...
这是让 widget 与应用视觉融合的关键。Sparkle 使用 OKLCH 色彩空间的 CSS 变量，而 Anthropic 的 widget 设计指南使用 `--color-background-primary` 这样的标准变量名。

桥接层在 iframe 初始化时将 Sparkle 的变量值注入 iframe 的 `:root`，模型按指南写的 CSS 就能直接使用当前主题的颜色。深色模式切换时，父页面检测到 `class` 变化，重新计算变量值并推送给 iframe。
