...
这四条对产品定位的启示：**Sparkle 的 Artifact 不是 Claude 那种"生成一次就扔"的一次性预览，而是和文件系统强绑定的持续编辑面板**。这个差异支配了后续所有设计。
...
- Claude 的 Artifact 是单轮对话产物，弹了就弹；Sparkle 的场景是多轮迭代，AI 一个 session 可能改十几个文件，每次都自动弹等于洗屏。
...
- (a) 增加二进制依赖 + 打包复杂度。
- (b) 数据跑出本机是隐私敏感问题（用户可能在导出内含敏感信息的 HTML），违背 Sparkle 的"本地优先"定位。
...
- 这不是 bug，是 feature：Sparkle 本身是 Electron app，dev server 是开发便利。
