...
11 个 issue 的共性根因。用户安装 Sparkle 后，里面的 SDK 调用的是系统上的 Claude Code CLI。如果 CLI 版本太旧（如 2.0.x），SDK 接口不兼容直接 exit code 1，用户看到的是"发送消息失败"。
...
考虑过让 Sparkle 自动更新 CLI，但放弃了：
- CLI 是系统级工具，不属于 Sparkle 管理范畴
- 自动更新可能打断正在进行的会话
...
1. **代理检测只在启动时执行一次** — 用户开关 VPN 后需要重启 Sparkle
2. **WinGet 检测有 5 秒超时** — 首次 status 检查可能略慢
3. **Homebrew/WinGet 无法确认是否有新版本** — 只能提供升级入口
4. **native 安装的"自动更新"可能不及时** — Sparkle 通过 SDK spawn 短生命周期子进程，自动更新机制来不及生效。用户仍可以手动运行 `claude update`
