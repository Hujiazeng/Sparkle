...
Sparkle 生成 summary 后，如果 SDK session 还在 resume，SDK 会用**自己**的 pre-compaction transcript，我们的 summary 根本不会到模型。所以任何 Sparkle-side 压缩（手动 /compact 或 auto pre-compression）成功后必须：
