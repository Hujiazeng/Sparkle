...
让用户通过 Telegram、Discord、飞书、微信等 IM 通道远程操控 Sparkle 中的 Claude 会话。Bridge 复用现有 `streamClaude()` 管线，在服务端直接消费 SSE 流，而不是依赖浏览器标签页。
...
| 表 | 用途 |
|---|------|
| channel_bindings | IM 地址 → Sparkle session 映射 |
