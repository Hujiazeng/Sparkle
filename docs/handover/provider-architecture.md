...
> 数据采集日期：2026-04-04
> 数据来源：18 个服务商官方文档 + Claude Code 真实源码（/资料/src） + Sparkle 代码库

---

## 一、架构总览

Sparkle 通过 **Provider Catalog → Provider Resolver → Claude Code SDK subprocess** 三层架构连接 AI 服务商：
...
## 三、Sparkle 如何桥接 Claude Code

### 桥接核心：`toClaudeCodeEnv()`

`toClaudeCodeEnv()`（`provider-resolver.ts`）把 Sparkle DB 里的 Provider 配置翻译成 Claude Code 期望的环境变量，注入给 SDK 子进程：
...
> 下表分两列：**官方文档推荐配置** vs **Sparkle 当前 preset 实际行为**。标 ⚠️ 的表示两者不一致。

### 高频服务商对齐状态

| 服务商 | 官方要求的认证方式 | Sparkle preset authStyle | 状态 | 差异说明 |
|--------|------------------|---------------------------|------|---------|
...
| Google AI Studio | `GEMINI_API_KEY` | `api_key`（media category） | ✅ | 一致；注：Sparkle 选择 AI Studio 路线，不跟随上游 Vertex 方案 |

### Issue 归因：服务商配置问题不全是 Sparkle 的责任

GitHub 上 100+ 个服务商相关 Issue 的归因分三类：

1. **~1/3 Sparkle preset 错误**（上表 ⚠️ 标注的项）— 修 preset authStyle 即可解决（历史版本也提 `PROVIDER_MANAGED_BY_HOST` 但已确认是死代码，见下方第四节 P0.1）
2. **~1/3 用户认知差**（以为终端 settings.json 配置能被 Sparkle 继承）— 需在配置引导中明确说明两套系统独立
...
Sparkle 的 Google 支持选择 **AI Studio (Gemini API)** 路线，不跟随上游 Claude Code 的 Vertex AI 方案。原因：Vertex 需要 GCP OAuth + 项目配置，门槛过高；AI Studio 只需一个 API Key。上游 Claude Code 原生支持 Vertex（`CLAUDE_CODE_USE_VERTEX`），这是上游能力，Sparkle 当前不实现。
...
但 preset 静态列表仍是大多数用户看到的来源，服务商出新模型后需要等 Sparkle 发版。
...
Claude Code 上游已支持 `@anthropic-ai/foundry-sdk`（`CLAUDE_CODE_USE_FOUNDRY`），Sparkle 缺少 `foundry` 协议。

#### 9. 3P 模型能力声明

上游支持通过 `ANTHROPIC_DEFAULT_*_MODEL_SUPPORTED_CAPABILITIES` 声明第三方模型能力。Sparkle 可以在 preset 中预设这些值，让 SDK 知道哪些第三方模型支持 thinking/effort 等功能。
