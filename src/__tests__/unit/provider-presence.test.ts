/**
 * Unit tests for hasSparkleProvider() — the precheck used by /api/chat to
 ...
 * source for Sparkle's provider-presence check. The user must have either
...
describe('hasSparkleProvider', () => {
  it('returns false on a clean install (no DB provider, no env, no settings)', async () => {
    const { hasSparkleProvider } = await import('../../lib/provider-presence');
    assert.equal(hasSparkleProvider(), false);
  });

  it('returns true when process.env.ANTHROPIC_API_KEY is set', async () => {
    process.env.ANTHROPIC_API_KEY = 'sk-test';
    const { hasSparkleProvider } = await import('../../lib/provider-presence');
    assert.equal(hasSparkleProvider(), true);
  });
  ...
  it('returns FALSE even when ~/.claude/settings.json has a cc-switch token — this is by design', async () => {
    ...
    // hasSparkleProvider() intentionally ignores it so users relying only on
    // cc-switch get routed to the Sparkle setup wizard. This will intercept
