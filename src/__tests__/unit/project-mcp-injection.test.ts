...
describe('loadProjectMcpServers — explicit project .mcp.json injection', () => {
  ...
  (process.env.CI ? it.skip : it)('resolves ${...} env placeholders against Sparkle DB settings', async () => {
    writeProjectMcpJson({
      mcpServers: {
        'team-mcp-with-token': {
          command: 'team-mcp',
    ...
    // Seed Sparkle DB with the secret value
    ...
    assert.equal(servers!['team-mcp-with-token'].env?.TEAM_API_TOKEN, 'sk-team-secret',
      '${...} placeholder must be resolved against Sparkle DB so team .mcp.json files can reference user-managed secrets');
  });
...
// ────────────────────────────────────────────────────────────────
//
// Sparkle's MCP Manager UI stores per-server enable/disable as
