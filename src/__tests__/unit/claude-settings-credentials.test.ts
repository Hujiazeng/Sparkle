 ...
 * the fix, Sparkle's `hasCredentialsForRequest()` only checked shell env and
...
// ── End-to-end chain: cc-switch user, no Sparkle provider ──
//
// Simulates the exact production scenario from #461 / #478:
//   1. Fresh Sparkle install — empty DB, no providers configured
...
describe('cc-switch end-to-end (no Sparkle provider, settings.json only)', () => {
