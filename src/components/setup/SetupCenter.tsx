...
import { DigitalHumanApiCard } from './DigitalHumanApiCard';
...
interface SetupCenterProps {
  onClose: () => void;
  initialCard?: 'digitalHuman' | 'project';
}

export function SetupCenter({ onClose, initialCard }: SetupCenterProps) {
  const { t } = useTranslation();
  const [digitalHumanStatus, setDigitalHumanStatus] = useState<SetupCardStatus>('not-configured');
  ...
  // (e.g. from a skipped card's settings link or
  ...
  const completedCount = [digitalHumanStatus, projectStatus]
  ...
  // modal at 2/2 (leave it visible, user will close it themselves).
  useEffect(() => {
    if (
      completedCount === 2 &&
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 backdrop-blur-sm">
      <div className="w-full max-w-lg max-h-[90vh] overflow-y-auto rounded-xl border bg-card shadow-2xl">
        ...
        <div className="sticky top-0 z-10 flex items-center justify-between bg-card px-6 pt-6 pb-3 border-b">
          ...
          <div className="flex items-center gap-3">
            <span className="text-xs text-muted-foreground">
              {t('setup.progress', { completed: String(completedCount), total: '2' })}
            </span>
        ...
        <div className="p-6 space-y-4">
          ...
          <div id="setup-card-digitalHuman">
            <DigitalHumanApiCard
              status={digitalHumanStatus}
              onStatusChange={setDigitalHumanStatus}
            />
          </div>
