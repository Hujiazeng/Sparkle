...
import { loadDigitalHumanAssets, refreshDigitalHumanAssets } from '@/lib/digital-human-assets-cache';
...
function isChatPath(pathname: string | null): boolean {
...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const [setupInitialCard, setSetupInitialCard] = useState<'digitalHuman' | 'project' | undefined>();
  ...
  useEffect(() => {
    loadDigitalHumanAssets();
    const handler = () => refreshDigitalHumanAssets();
  ...
  useEffect(() => {
    const handler = (e: Event) => {
      ...
      setSetupInitialCard(detail?.initialCard === 'project' ? 'project' : 'digitalHuman');
      setSetupOpen(true);
    };
  ...
  // Hash bridge: error messages render `[Open Settings](/settings#digital-human)`
  ...
  // such a link is clicked the hash changes to `#digital-human`, and we surface
  // the SetupCenter Digital Human card here — EXCEPT on /settings itself, where
  // SettingsLayout owns the hash for its own section routing
  ...
  useEffect(() => {
    const maybeOpenFromHash = () => {
      ...
      if (window.location.hash === '#digital-human') {
        setSetupInitialCard('digitalHuman');
        setSetupOpen(true);
  ...
  const [splitSessions, setSplitSessions] = useState<SplitSession[]>(() => (
    isChatPath(pathname) ? loadSplitSessions() : []
  ));
  const [activeColumnId, setActiveColumnIdRaw] = useState<string>(() => (
    isChatPath(pathname) ? loadActiveColumn() : ""
  ));
  ...
  useEffect(() => {
    if (isSplitActive && !isChatPath(pathname)) {
