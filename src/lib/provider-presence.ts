/**
 * provider-presence.ts — Single-source-of-truth: "does Sparkle itself have
 ...
 *     lives under the Claude Code CLI's ownership; Sparkle treats CLI login
 *     state as "not our business". A user with only settings.json and no
 *     Sparkle-level provider will be intercepted and asked to add one.
 *
 * Used by:
...
/**
 * True when a single DB provider has credentials Sparkle can dispatch on.
 *
 * Checks (in order):
 *   1. api_key non-empty
 ...
 */
export function providerHasUsableSparkleAuth(p: ApiProvider): boolean {
...
/**
 * True when Sparkle has at least one provider the backend can dispatch to.
 *
...
export function hasSparkleProvider(): boolean {
  ...
  try {
    for (const p of getAllProviders()) {
      if (providerHasUsableSparkleAuth(p)) return true;
    }
  } catch {
    return true;
  }
