/**
 * Telegram Bot notification module for Sparkle.
 *
...
export async function notifyPermissionRequest(
  ...
  const msg = [
    ...
    ``,
    `⚠️ Please approve or deny in Sparkle.`,
  ].filter(Boolean).join('\n');
...
/**
 * Build a status summary of current Sparkle sessions.
 ...
 * Supports: /status — replies with current Sparkle session status.
        ...
        if (command === '/start') {
          await callTelegramApi(botToken, 'sendMessage', {
            ...
            text: [
              `👋 <b>Sparkle Bot</b>`,
              ``,
              `I'll send you notifications about your Sparkle tasks.`,
              ``,
              `<b>Commands:</b>`,
