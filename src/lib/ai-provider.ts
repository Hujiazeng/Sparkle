...
export function createModel(opts: CreateModelOptions = {}): CreateModelResult {
  ...
  if (!resolved.hasCredentials && !resolved.provider) {
    ...
    if (hasClaudeSettingsCredentials()) {
      throw new Error(
        'Credentials found in ~/.claude/settings.json (managed by cc-switch or similar), but the Native runtime cannot read them. Switch the runtime to "Claude Code SDK" in Settings → Runtime, or add the provider to Sparkle directly.',
      );
    }
    throw new Error(
