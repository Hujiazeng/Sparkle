        ...
        async ({ toolId, name: toolName }) => {
          try {
            ...
            const warning = provenanceIsGuessed
              ? `\n\nNote: The install method was guessed (${updateMethod}) because this tool was installed outside Sparkle. If the update failed, it may have been installed via a different package manager.`
              : '';
            return {
              content: [{
                type: 'text' as const,
