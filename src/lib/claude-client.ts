    ...
    async start(controllerRaw) {
      ...
      try {
        ...
        const queryOptions: Options = {
          ...
          settingSources: resolved.settingSources as Options['settingSources'],
          // Auto-allow all Sparkle built-in MCPs. These are host-defined
          // in-process servers (createSdkMcpServer in claude-client.ts below)
          // that ship with Sparkle — they're not third-party plugins and
        ...
        // MCP servers: pass explicitly provided config (e.g. from Sparkle UI).
        ...
        if (resolved.provider) {
          ...
          if (projectMcps) {
            ...
            // Existing entries (Sparkle UI / placeholder-managed) take
        ...
        // Sparkle does NOT explicitly inject plugins — the SDK reads settingSources
        ...
        queryOptions.canUseTool = async (toolName, input, opts) => {
          // Auto-approve Sparkle's own in-process MCP tools — they are internal
