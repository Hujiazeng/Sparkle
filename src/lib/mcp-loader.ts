 ...
 * We only manually pass servers that need Sparkle-specific processing:
 * ${...} env placeholder resolution from the Sparkle DB.
 *
...
/**
 * Load MCP servers that need Sparkle-specific processing.
 *
 * Returns only servers with ${...} env placeholders that were resolved
 * against the Sparkle DB. Returns undefined when no such servers exist
 * (the common case), letting the SDK load everything natively.
 *
 * Used by: route.ts, conversation-engine.ts — passed to streamClaude().
 */
export function loadSparkleMcpServers(): Record<string, MCPServerConfig> | undefined {
  try {
 ...
 * NOT intended for passing to the SDK — use loadSparkleMcpServers() instead.
 *
 ...
 * Sparkle DB the same way loadAndMerge does. Disabled servers are
 * filtered out.
 *
...
export function loadProjectMcpServers(projectCwd: string | undefined): Record<string, MCPServerConfig> | undefined {
  ...
  try {
    ...
    // The Sparkle MCP Manager UI persists per-server enable/disable state
