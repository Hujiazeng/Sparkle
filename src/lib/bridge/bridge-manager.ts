    ...
    case '/feishu': {
      ...
      switch (subcommand) {
        case 'start': {
          ...
          if (!config) {
            response = '❌ Feishu plugin not configured.\n\nPlease set App ID and App Secret in Sparkle settings, or use /feishu auth.';
            break;
          }
        ...
        case 'auth': {
          ...
          const config = (plugin as FeishuChannelPlugin).getConfig?.();
          if (!config) {
            response = '❌ App credentials not configured.\n\nPlease configure App ID and App Secret in Sparkle Settings → Bridge → Feishu.';
            break;
          }
          // Note: Sparkle currently uses app-level bot tokens (no user OAuth)
          ...
          response = [
            ...
            '',
            'ℹ️ Sparkle uses app-level bot tokens.',
        ...
        case 'doctor': {
          ...
          // Known limitations
          lines.push('');
          lines.push('Known Limitations (Sparkle vs OpenClaw):');
