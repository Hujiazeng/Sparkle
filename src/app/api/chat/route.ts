...
import { loadSparkleMcpServers, loadAllMcpServers } from '@/lib/mcp-loader';
...
import { hasSparkleProvider } from '@/lib/provider-presence';
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Precondition: Sparkle must have a provider configured. ~/.claude/settings.json
    // (cc-switch, CLI login) is intentionally NOT counted — users with only that source
    // are redirected to the setup flow to add a proper Sparkle provider.
    if (!hasSparkleProvider()) {
      return new Response(
        JSON.stringify({
          error: 'No provider configured in Sparkle.',
